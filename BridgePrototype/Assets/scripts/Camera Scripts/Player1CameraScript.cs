...
public class Player1CameraScript : MonoBehaviour {
	...
	void Update () 
	{
		...
		/*if (lerping) {
			...
			if(timer<lerpTime){
				...
				oldPos = player1.transform.position -(prevClosestPoint.GetComponent<PPointScript> ().direction * prevClosestPoint.GetComponent<PPointScript> ().distance);

				newPos = player1.transform.position -(closestPoint.GetComponent<PPointScript> ().direction * prevClosestPoint.GetComponent<PPointScript> ().distance);
		...
		if (!lerping) {
			...
			transform.position = player1.transform.position -(closestPoint.GetComponent<PPointScript> ().direction * closestPoint.GetComponent<PPointScript> ().distance);
		}*/

		transform.position = player1.transform.position - (direction*Distance);
	...
	/*void checkPPoints(){
		...
		foreach(GameObject point in points){
			if((player1.transform.position-point.transform.position).magnitude<minLength){
				minLength=(player1.transform.position-point.transform.position).magnitude;
